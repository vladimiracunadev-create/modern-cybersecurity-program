# CompTIA CySA+

> **Código:** CS0-003 · **Proveedor:** CompTIA · **Nivel:** Intermedio (defensivo) · [Página oficial](https://www.comptia.org/certifications/cybersecurity-analyst)

[⬅️ Volver al índice de certificaciones](README.md)

Analista de ciberseguridad: operaciones de seguridad, gestión de vulnerabilidades e incident response. Alineado con la ruta Blue Team.

## 📊 Cobertura estimada del programa: **88%**

`█████████░` 88.2% — suma ponderada por dominio (pesos oficiales del examen).

## Mapeo por dominio

| Dominio del examen | Peso | Partes del programa | Cobertura | Notas |
|---|---:|---|---:|---|
| Security Operations | 33% | Parte 1, 6, 8 | 88% | Telemetría, SIEM, detección, threat hunting, análisis de malware. |
| Vulnerability Management | 30% | Parte 3, 11, 17 | 92% | Ciclo de VM con CVSS/EPSS/KEV y SLAs (clase 318). |
| Incident Response and Management | 20% | Parte 9 | 90% | Ciclo NIST/SANS, contención, erradicación, RCA. |
| Reporting and Communication | 17% | Parte 3, 8 | 80% | Reporte de hallazgos y de detección. |
| **Total ponderado** | **100%** | | **88%** | |

## 🎯 Brecha y cómo cerrarla

La gestión de vulnerabilidades se toca en varias partes; conviene un repaso integrado (Parte 11 clase 245).

## 🧭 Ruta sugerida

Sigue la ruta 'Analista SOC / Blue Team' (partes 0,1,6,8,9). Ver [rutas por rol](../rutas/README.md) y la [Parte 16 — preparación de certificaciones](../classes/parte-16-capstones-y-preparacion-de-certificaciones/README.md).

---

> ⚠️ Mapeo orientativo, no avalado por el proveedor. La cobertura del temario no garantiza aprobar: las certificaciones prácticas requieren laboratorios y experiencia.
